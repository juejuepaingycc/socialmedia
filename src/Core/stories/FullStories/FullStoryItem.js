...
export default class FullStoryItem extends React.Component {
  constructor(props) {
    super(props);
    console.log("Item>>"+ JSON.stringify(this.props.story))
    this.state = {
  ...
  deleteStory = async () => {
    ...
    console.log("delete..." + story.items[story.idx].src)

    const res = await firebasePost.deleteStory(story.items[story.idx].src, true);
  ...
  renderDeleteButton() {
    return (
      <TouchableWithoutFeedback onPress={this.deleteStory}>
        <View style={[styles.closeButton, styles.deleteButton]}>
          <Icon name='delete-outline' size={23} />
        </View>
      </TouchableWithoutFeedback>
    );
  }
